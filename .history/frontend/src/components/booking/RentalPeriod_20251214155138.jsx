...
const RentalPeriod = ({ rentalData, setRentalData, bookedDates }) => {
  ...
  const handleStartDateChange = (date) => {
    ...
    if (dayjs(start).isBefore(dayAfterTomorrow)) {
      setErrors((prev) => ({
        ...prev,
        startDate: "Start date must be at least 2 days from now.",
      }));
    } else {
    ...
    if (rentalData.durationValue > 0) {
      autoComputeEndDate(
        start,
        rentalData.durationValue,
        rentalData.durationType
      );
    }
  };
  ...
  const handleEndDateChange = (date) => {
    ...
    if (
      ...
      (endDate.isBefore(startDate) || endDate.isSame(startDate, "day"))
    ) {
      setErrors((prev) => ({
        ...prev,
      ...
      setErrors((prev) => ({
        ...prev,
        endDate: "Selected date range overlaps with an existing booking.",
      }));
    } else {
                ...
                textField: {
                  ...
                  sx: {
                    "& .MuiOutlinedInput-root": {
                      ...
                      "& .MuiInputBase-input": {
                        padding: "14px 14px", // 👈 vertical spacing inside
                        fontSize: "15px",
                      },
                    },
                  },
