...
const CollectionCard = ({
  ...
  // 🔹 Prevent rendering if itemData is missing
  ...
  const canCancel = () => {
    ...
    return diffDays >= 1 && (status === "pending" || status === "approved");
  };
    ...
    >
      <div className="flex gap-6 relative">
          ...
          <div>
            ...
            <div className="text-[13px] mt-1 text-gray-700">
              <div className="flex justify-start items-center text-[13px] text-gray-600 mt-1">
                <span>Listed by {itemData?.owner?.name || "Unknown"}</span>
                <span className="flex items-center ml-4 gap-1 text-[13px] text-gray-600">
                  <MapPin size={14} className="text-gray-500" />{" "}
                  {itemData?.zone || "Unknown Zone"}, {itemData?.location || "Unknown Location"},{" "}
                  {itemData?.province || "Unknown Province"}
                </span>
              </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {status !== "approved" && status !== "pending" && (
                    <div className="flex items-center gap-1 text-gray-800 text-sm mb-3">
                      <Calendar size={15} />
                      {itemData?.minimumRentalDays && itemData?.maximumRentalDays
                        ? `${itemData.minimumRentalDays} - ${itemData.maximumRentalDays} days`
                        : itemData?.daysAvailable || itemData?.days || itemData?.availableDays}{" "}
                      available
                    </div>
                  )}
                  ...
                  <div className="flex items-center text-[12px] gap-2 text-gray-500 mt-1">
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {itemData?.shipping > 0
                          ? `Delivery (₱${Math.round(itemData.shipping)})`
                          : "Delivery (Free)"}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>Security Deposit (₱{itemData?.securityDeposit || 0})</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right text-[13px] flex flex-col gap-0.5">
                  ...
                  {(status === "approved" || status === "pending") &&
                    (() => {
                      ...
                      const totalWithDeposit = itemTotals.total + (itemData?.securityDeposit || 0);
                      return (
                        <div className="flex justify-between w-full text-[13px] gap-1">
                          ...
                          <span className="text-gray-500">
                            Total{" "}
                            <span className="font-semibold text-gray-900">{`₱${totalWithDeposit}`}</span>
                          </span>
                        </div>
                      );
                    })()}
        ...
        <div className="absolute bottom-1 right-0 flex items-center gap-1.5">
          ...
          {status === "approved" ? (
            <button
              onClick={() => alert(`Contacting owner: ${itemData?.owner?.name}`)}
