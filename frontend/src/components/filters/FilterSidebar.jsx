      ...
      <aside className="w-80 bg-white  text-purple-900   shadow-md border border-gray-200 rounded-2xl h-[815px] flex flex-col">
        <div className="flex-1 overflow-y-auto px-6 pt-6 space-y-3 pb-0">
          ...
          <div className="flex justify-between items-center mb-5">
            <h2 className="text-[16px] font-semibold text-purple-900">
              Filter
            </h2>
            <button
          ...
          {/* Category */}
          <div>
            <p className="font-light text-[13px] text-gray-700 mb-2">
              Category
            </p>
            <div
              ref={scrollRef}
            ...
            >
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-5 py-1.5 text-[12px] rounded-full whitespace-nowrap border transition ${
          ...
          <div>
            ...
            <div
              className={`w-full border border-gray-300 rounded-lg px-3 py-2 mb-5 text-[13px] cursor-pointer hover:bg-purple-50 transition ${
                location ? "text-gray-700" : "text-gray-400 opacity-70"
              }`}
            ...
            <div className="flex flex-nowrap justify-between mt-3 mb-6 space-x-2">
              {["Today", "This Week", "This Month"].map((range) => (
                <button
                  ...
                  className={`flex-1 text-[12px] px-4 py-2 rounded-lg border transition whitespace-nowrap ${
                    selectedRange === range
                      ? "bg-[#7A1CA9] text-white border-[#7A1CA9] "
            ...
            <div className="flex flex-col space-y-2 pointer-events-auto">
              {[5, 4, 3, 2, 1].map((r) => (
                <button
                  key={r}
                  onClick={() => setRating(r)}
                  className={`flex items-center justify-between gap-2 p-2 border rounded-lg hover:bg-gray-100 transition text-sm cursor-pointer ${
                    rating === r
                  ...
                  <div className="flex">
                    {Array.from({ length: 5 }).map((_, index) => (
                      <span
                        key={index}
                        className={`text-[15px] ${
                          index < r ? "text-yellow-400" : "text-gray-300"
                        }`}
                      >
                        ★
                      </span>
